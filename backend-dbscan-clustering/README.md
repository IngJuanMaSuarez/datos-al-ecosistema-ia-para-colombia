# Backend de Clustering DBSCAN (Turf.js)
...
desde un **widget de ArcGIS Experience Builder**, aplica el algoritmo **DBSCAN**
con [`@turf/clusters-dbscan`](https://turfjs.org/docs/#clustersDbscan) y devuelve
los resultados etiquetados para visualización interactiva en el frontend.
...
graph TD
    ...
    subgraph Librerias["Bibliotecas Externas"]
        Turf("@turf/clusters-dbscan")
        Helpers("@turf/helpers")
    end

    Widget -->|"POST /api/cluster<br/>{points, radius}"| API
    API -->|"valida y reenvía"| DBSCAN
    DBSCAN -->|"clustersDbscan()"| Turf
    DBSCAN -->|"featureCollection()"| Helpers
...
| **@turf/clusters-dbscan** | 7.1 | Implementación DBSCAN sobre GeoJSON |
...
- **Radio configurable**: el parámetro `radius` se define en **metros** y se pasa
  como `maxDistance` a DBSCAN.
